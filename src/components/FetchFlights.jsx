  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const originAirport = airportsData.find(
        airport => 
          airport.cityName.toLowerCase() === searchParams.origin.toLowerCase() ||
          airport.airportCode.toLowerCase() === searchParams.origin.toLowerCase() ||
          `${airport.cityName} (${airport.airportCode})`.toLowerCase() === searchParams.origin.toLowerCase()
      );

      const destinationAirport = airportsData.find(
        airport => 
          airport.cityName.toLowerCase() === searchParams.destination.toLowerCase() ||
          airport.airportCode.toLowerCase() === searchParams.destination.toLowerCase() ||
          `${airport.cityName} (${airport.airportCode})`.toLowerCase() === searchParams.destination.toLowerCase()
      );
      ...
      if (!destinationAirport) {
        setError(`Aéroport de destination "${searchParams.destination}" non trouvé`);
        setLoading(false);
        return;
      }
      ...
      if (!data.data || data.data.length === 0) {
        setError(`Aucun vol trouvé entre ${originAirport.cityName} et ${destinationAirport.cityName}`);
      } else {
    ...
    <div className="bg-white">
      ...
      {flights.length > 0 && (
        <div className="px-20 py-10">
          <h3 className="text-2xl font-semibold mb-4 text-colorB">Offres de vol ({flights.length} résultats)</h3>
