  ...
  const transformFlights = (apiFlights) => {
    return apiFlights.map((flight, index) => {
      ...
      return {
        ...
        returnDepartureCode: flight.itineraries[1]
          ? flight.itineraries[1].segments[0].departure.iataCode
          : null,
        returnDepartureCity: flight.itineraries[1]
          ? findCityName(flight.itineraries[1].segments[0].departure.iataCode)
          : null,
        returnArrivalCode: lastReturnSegment
          ? lastReturnSegment.arrival.iataCode
          : null,
        returnArrivalCity: lastReturnSegment
          ? findCityName(lastReturnSegment.arrival.iataCode)
          : null,
        returnDepartureTime: flight.itineraries[1]
          ? formatDateTime(flight.itineraries[1].segments[0].departure.at)
          : null,
        returnArrivalTime: lastReturnSegment
          ? formatDateTime(lastReturnSegment.arrival.at)
          : null,
        returnDuration: flight.itineraries[1]
          ? calculateFlightDuration(flight.itineraries[1].segments)
          : null,
        returnStops: flight.itineraries[1]
          ...
          <div className="grid grid-cols-4 mt-8">
            ...
            >
              ...
              <p className="font-thin">
                {vol.departureCode} - {vol.departureCity}
              </p>
            </div>
            ...
            >
              <p>{vol.arrivalTime}</p>
              <p className="font-thin">
                {vol.arrivalCode} - {vol.arrivalCity}
              </p>
              <p className="text-sm text-gray-500">
                {vol.outboundDuration}
                {vol.outboundStops > 0 &&
                  ` • ${vol.outboundStops} escale${
                    vol.outboundStops > 1 ? "s" : ""
                  }`}
              </p>
            </div>
          </div>
          ...
          {vol.returnDepartureCode && (
            <div className="grid grid-cols-4 mt-8">
              ...
              >
                ...
                <p className="font-thin">
                  {vol.returnDepartureCode} - {vol.returnDepartureCity}
                </p>
              </div>
              ...
              >
                ...
                <p className="font-thin">
                  {vol.returnArrivalCode} - {vol.returnArrivalCity}
                </p>
                <p className="text-sm text-gray-500">
                  {vol.returnDuration}
                  {vol.returnStops > 0 &&
                    ` • ${vol.returnStops} escale${
                      vol.returnStops > 1 ? "s" : ""
                    }`}
                </p>
              </div>
